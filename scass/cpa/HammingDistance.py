...
def hammingDistanceCorrolation(traces,inputsA, inputsB):
    """
    Compute the hamming distance corrolation trace from the given trace
    set and input values.

    traces - an np.NdArray where each row is a single trace.
    inputA - an np.NdArray where each i'th element is the input for each
             i'th trace row.
    inputB - an np.NdArray where each i'th element is the input for each
             i'th trace row.
    """
